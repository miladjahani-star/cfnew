...
// ===== 多用户管理（KV users：管理员创建子账号出售给终端用户）=====
let 用户列表缓存 = null;
let 用户列表缓存时间 = 0;
let 用户列表版本 = 0;
let 有效用户UUID集合 = null;
let 有效用户SHA224集合 = null;
let 有效用户SHA224版本 = -1;

async function 读取用户列表(强制 = false) {
  if (!键值存储) return [];
  if (!强制 && 用户列表缓存 && Date.now() - 用户列表缓存时间 < 2000) return 用户列表缓存;
  try {
    const 数据 = await 键值存储.get('users');
    用户列表缓存 = 数据 ? JSON.parse(数据) : [];
    if (!Array.isArray(用户列表缓存)) 用户列表缓存 = [];
    用户列表缓存时间 = Date.now();
  } catch (错误) {
    if (!用户列表缓存 || !Array.isArray(用户列表缓存)) 用户列表缓存 = [];
  }
  return 用户列表缓存;
}
async function 保存用户列表(列表) {
  用户列表缓存 = Array.isArray(列表) ? 列表 : [];
  用户列表缓存时间 = Date.now();
  用户列表版本++;
  if (键值存储) await 键值存储.put('users', JSON.stringify(用户列表缓存));
}
async function 刷新有效用户集合() {
  try {
    const 列表 = await 读取用户列表();
    const 集合 = new Set();
    for (const 用户 of 列表) {
      if (用户.enabled !== false && 用户.uuid && 是否有效格式(用户.uuid)) {
        集合.add(用户.uuid.toLowerCase());
      }
    }
    有效用户UUID集合 = 集合;
    if (!传输路径 && 集合.size > 0 && 有效用户SHA224版本 !== 用户列表版本) {
      const 哈希集合 = new Set();
      for (const 标识 of 集合) {
        try {
          哈希集合.add(await 处理值224井号(标识));
        } catch (忽略) {}
      }
      有效用户SHA224集合 = 哈希集合;
      有效用户SHA224版本 = 用户列表版本;
    ...
    if (!有效用户UUID集合) 有效用户UUID集合 = new Set();
  }
}
function 生成新唯一标识() {
  try {
    ...
    const 值 = [];
    for (let 索引 = 0; 索引 < 32; 索引++) 值.push(Math.floor(Math.random() * 16).toString(16));
    const 字符串 = 值.join('');
    return 字符串.slice(0, 8) + '-' + 字符串.slice(8, 12) + '-4' + 字符串.slice(13, 16) + '-' + ['8', '9', 'a', 'b'][Math.floor(Math.random() * 4)] + 字符串.slice(17, 20) + '-' + 字符串.slice(20, 32);
  }
}
function 是否有效用户唯一标识(字节, 偏移 = 0) {
  if (!有效用户UUID集合 || 有效用户UUID集合.size === 0) return false;
  try {
    return 有效用户UUID集合.has(处理格式值(字节, 偏移));
  } catch (忽略) {
    return false;
  }
}
async function 是否为已启用用户(唯一标识) {
  const 列表 = await 读取用户列表();
  const 匹配 = 列表.find(项 => (项.uuid || '').toLowerCase() === (唯一标识 || '').toLowerCase());
  return !!匹配 && 匹配.enabled !== false;
}
  ...
  async fetch(请求735, 本地值734, 本地值733) {
    try {
      ...
      if (!是否网页套接字 && !是否值732 && 请求网址731.pathname !== '/') {
        ...
        // 多用户支持：UUID 格式的首段放行，后续在路由中校验是否为已注册用户
        if (首次值 !== 值值728 && (清理值 ? 首次值 !== 清理值 : false) && !是否有效格式(首次值)) {
      ...
      if (网址698.pathname.includes('/api/users')) {
        const 路径部分列表699 = 网址698.pathname.split('/').filter(参数值698 => 参数值698);
        const 接口索引697 = 路径部分列表699.indexOf('api');
        if (接口索引697 > 0) {
          const 路径值696 = 路径部分列表699.slice(0, 接口索引697);
          const 路径值695 = 路径值696.join('/');
          let 是否有效694 = false;
          if (自定义路径 && 自定义路径.trim()) {
            const 清理自定义路径693 = 自定义路径.trim().startsWith('/') ? 自定义路径.trim().substring(1) : 自定义路径.trim();
            是否有效694 = 路径值695 === 清理自定义路径693;
          } else {
            是否有效694 = 是否有效格式(路径值695) && 路径值695 === 认证令牌;
          }
          if (是否有效694) {
            return await 处理用户接口(请求735);
          } else {
      ...
      if (事件值.key === 'Enter') {
        ...
        if (自定义路径 && 自定义路径.trim()) {
          ...
          if (网址698.pathname.length > 1 && 网址698.pathname !== '/') {
            ...
            if (是否有效格式(用户658)) {
              // 多用户支持：自定义路径模式下，已注册且启用的用户 UUID 路径仍然有效
              if (await 是否为已启用用户(用户658)) {
                if (网址698.pathname.endsWith('/sub')) {
                  return await 处理订阅请求(请求735, 用户658, 网址698);
                }
                return await 处理订阅值(请求735, 用户658);
              }
              return new Response(JSON.stringify({
                error: '访问被拒绝',
                message: 'UUID未注册或已停用'
...
function 解析网页套接字值头部(块297, 令牌) {
  ...
  if (!处理值唯一标识(字节296, 1, 令牌) && !是否有效用户唯一标识(字节296, 1)) return {
    hasError: true,
    message: 错误_无效用户
  };
...
async function 处理订阅值(请求241, 用户240 = null) {
  ...
  const 本地值235 = {
    zh: {
      ...
      userManagement: '用户管理',
      userManagementDesc: '创建子账号出售给终端用户，每个用户拥有独立 UUID 和订阅链接',
      createUser: '创建用户',
      userNamePlaceholder: '用户名称（可选）',
      userNote: '提示：用户订阅链接为 https://你的域名/用户UUID/sub',
      noUsers: '暂无用户，点击上方按钮创建',
      userNameCol: '名称',
      userStatusCol: '状态',
      userActions: '操作',
      userEnabled: '启用',
      ...
      disableUser: '停用',
      deleteUser: '删除',
      confirmDeleteUser: '确定要删除该用户吗？删除后该用户将无法再使用订阅。',
      userCreated: '用户创建成功',
      userDeleted: '用户已删除',
      userUpdated: '用户状态已更新',
      userLoadFailed: '加载用户列表失败',
      userCreateFailed: '创建用户失败',
      userUpdateFailed: '操作失败',
      uuidCopied: 'UUID 已复制',
      subCopied: '订阅链接已复制',
      copyFailed: '复制失败，请手动复制',
      kvRequiredForUsers: '需要 KV 存储才能管理用户',
      loadingUsers: '加载中...'
    },
    fa: {
      title: 'مرکز اشتراک',
      ...
      userManagementDesc: 'حساب‌های کاربری بسازید و مدیریت کنید تا به کاربران نهایی بفروشید؛ هر کاربر UUID و لینک اشتراک مستقل دارد',
      createUser: 'ایجاد کاربر',
      userNamePlaceholder: 'نام کاربر (اختیاری)',
      userNote: 'نکته: لینک اشتراک کاربر: https://دامنه‌ی شما/UUID کاربر/sub',
      noUsers: 'کاربری وجود ندارد، با دکمه بالا بسازید',
      userNameCol: 'نام',
      userStatusCol: 'وضعیت',
      userActions: 'عملیات',
      userEnabled: 'فعال',
      userDisabled: 'غیرفعال',
      copySub: 'کپی اشتراک',
      copyUuid: 'کپی UUID',
      enableUser: 'فعال‌سازی',
      disableUser: 'غیرفعال‌سازی',
      deleteUser: 'حذف',
      confirmDeleteUser: 'آیا از حذف این کاربر مطمئن هستید؟ پس از حذف، اشتراک او کار نخواهد کرد.',
      userCreated: 'کاربر با موفقیت ایجاد شد',
      userDeleted: 'کاربر حذف شد',
      userUpdated: 'وضعیت کاربر به‌روزرسانی شد',
      userLoadFailed: 'بارگذاری لیست کاربران ناموفق',
      userCreateFailed: 'ایجاد کاربر ناموفق',
      userUpdateFailed: 'عملیات ناموفق',
      uuidCopied: 'UUID کپی شد',
      subCopied: 'لینک اشتراک کپی شد',
      copyFailed: 'کپی ناموفق، لطفا دستی کپی کنید',
      kvRequiredForUsers: 'برای مدیریت کاربران به ذخیره‌سازی KV نیاز است',
        ...
        <div class="container">
            ...
            <div class="card" id="configCard" style="display: none;">
                ...
                <div id="configContent" style="display: none;">
                    ...
                    <div id="userManagement" style="background: rgba(15, 3, 40, 0.7); border: 1px solid #00f0ff; padding: 15px; margin: 10px 0; font-family: 'Courier New', monospace;">
                            <div style="font-weight: bold; margin-bottom: 8px; color: #00ff9d; text-shadow: 0 0 5px #00ff9d;">${翻译值.userManagement}</div>
                            <div style="color: #7aa9c4; font-size: 0.85rem; margin-bottom: 12px;">${翻译值.userManagementDesc}</div>
                            <div style="color: #7aa9c4; font-size: 0.8rem; margin-bottom: 12px;">${翻译值.userNote}</div>
                            <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 15px;">
                                <input type="text" id="newUserName" placeholder="${翻译值.userNamePlaceholder}" style="flex: 1; min-width: 180px; padding: 10px; background: rgba(0, 0, 0, 0.8); border: 1px solid #00f0ff; color: #00f0ff; font-family: 'Courier New', monospace; font-size: 13px;">
                                <button type="button" onclick="创建新用户()" style="padding: 10px 18px; background: linear-gradient(135deg, #00f0ff, #00b8c4); color: #05030e; border: none; cursor: pointer; font-weight: bold; font-family: 'Courier New', monospace;">${翻译值.createUser}</button>
                            </div>
                            <div id="userList" style="color: #7aa9c4; font-size: 0.85rem;">
                                <div>${翻译值.loading}</div>
                            </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (window.是否管理员) {
    ...
    var 订阅地址 = window.location.href.replace(/\/+$/, '') + '/sub';
    var 订阅显示 = document.getElementById('clientSubscriptionUrl');
    if (订阅显示) {
      订阅显示.textContent = 订阅地址;
      订阅显示.style.display = 'block';
      订阅显示.style.overflowWrap = 'break-word';
      订阅显示.style.wordBreak = 'break-all';
      订阅显示.style.maxWidth = '100%';
      订阅显示.style.boxSizing = 'border-box';
    }
  }
...
// ===== 用户管理（多用户：管理员创建子账号出售给终端用户）=====
async function 加载用户列表() {
  const 容器 = document.getElementById('userList');
  if (!容器) return;
  容器.innerHTML = '<div style="color: #7aa9c4;">' + 翻译值20207.loadingUsers + '</div>';
  try {
    const 响应 = await fetch(window.location.pathname + '/api/users');
    if (响应.status === 503) {
      容器.innerHTML = '<div style="color: #ffb400;">' + 翻译值20207.kvRequiredForUsers + '</div>';
      return;
    }
    if (!响应.ok) {
      容器.innerHTML = '<div style="color: #ff3860;">' + 翻译值20207.userLoadFailed + ' (' + 响应.status + ')</div>';
      return;
    }
    const 数据 = await 响应.json();
    if (!数据.success) {
      容器.innerHTML = '<div style="color: #ff3860;">' + (数据.message || 翻译值20207.userLoadFailed) + '</div>';
      return;
    }
    const 列表 = 数据.data || [];
    if (列表.length === 0) {
      容器.innerHTML = '<div style="color: #7aa9c4;">' + 翻译值20207.noUsers + '</div>';
      return;
    }
    let html = '<table style="width: 100%; border-collapse: collapse; font-size: 12px;">';
    html += '<tr style="color: #00f0ff; text-align: left;">';
    html += '<th style="padding: 8px; border-bottom: 1px solid rgba(0, 240, 255, 0.3);">' + 翻译值20207.userNameCol + '</th>';
    html += '<th style="padding: 8px; border-bottom: 1px solid rgba(0, 240, 255, 0.3);">UUID</th>';
    html += '<th style="padding: 8px; border-bottom: 1px solid rgba(0, 240, 255, 0.3);">' + 翻译值20207.userStatusCol + '</th>';
    html += '<th style="padding: 8px; border-bottom: 1px solid rgba(0, 240, 255, 0.3);">' + 翻译值20207.userActions + '</th>';
    html += '</tr>';
    for (let 索引 = 0; 索引 < 列表.length; 索引++) {
      const 用户 = 列表[索引];
      html += '<tr style="border-bottom: 1px solid rgba(0, 240, 255, 0.15);">';
      html += '<td style="padding: 8px; color: #e6f5ff;">' + (用户.name || '-') + '</td>';
      html += '<td style="padding: 8px; color: #00f0ff; word-break: break-all;">' + 用户.uuid + '</td>';
      html += '<td style="padding: 8px;">' + (用户.enabled === false ? '<span style="color: #ff3860;">' + 翻译值20207.userDisabled + '</span>' : '<span style="color: #00ff9d;">' + 翻译值20207.userEnabled + '</span>') + '</td>';
      html += '<td style="padding: 8px; white-space: nowrap;">';
      html += '<button type="button" onclick="复制文本(\'' + 用户.subUrl + '\', \'sub\')" style="padding: 5px 9px; margin-right: 5px; background: rgba(0, 240, 255, 0.15); border: 1px solid #00f0ff; color: #00f0ff; cursor: pointer; font-family: Courier New, monospace; font-size: 11px;">' + 翻译值20207.copySub + '</button>';
      html += '<button type="button" onclick="复制文本(\'' + 用户.uuid + '\', \'uuid\')" style="padding: 5px 9px; margin-right: 5px; background: rgba(163, 71, 255, 0.15); border: 1px solid #a347ff; color: #a347ff; cursor: pointer; font-family: Courier New, monospace; font-size: 11px;">' + 翻译值20207.copyUuid + '</button>';
      html += '<button type="button" onclick="切换用户状态(\'' + 用户.uuid + '\', ' + (用户.enabled === false ? 'true' : 'false') + ')" style="padding: 5px 9px; margin-right: 5px; background: rgba(255, 178, 0, 0.12); border: 1px solid #ffb400; color: #ffb400; cursor: pointer; font-family: Courier New, monospace; font-size: 11px;">' + (用户.enabled === false ? 翻译值20207.enableUser : 翻译值20207.disableUser) + '</button>';
      html += '<button type="button" onclick="删除用户(\'' + 用户.uuid + '\')" style="padding: 5px 9px; background: rgba(255, 56, 96, 0.12); border: 1px solid #ff3860; color: #ff3860; cursor: pointer; font-family: Courier New, monospace; font-size: 11px;">' + 翻译值20207.deleteUser + '</button>';
      html += '</td></tr>';
    }
    html += '</table>';
    容器.innerHTML = html;
  } catch (错误) {
    容器.innerHTML = '<div style="color: #ff3860;">' + 翻译值20207.userLoadFailed + ': ' + 错误.message + '</div>';
  }
}
async function 创建新用户() {
  const 名称输入 = document.getElementById('newUserName');
  const 名称 = 名称输入 ? 名称输入.value.trim() : '';
  try {
    const 响应 = await fetch(window.location.pathname + '/api/users', {
      method: 'POST',
      headers: {
    ...
    const 数据 = await 响应.json();
    if (!响应.ok || !数据.success) {
      显示提示(数据.message || 翻译值20207.userCreateFailed, 'error');
      return;
    }
    if (名称输入) 名称输入.value = '';
    显示提示(翻译值20207.userCreated, 'success');
    await 加载用户列表();
  } catch (错误) {
    显示提示(翻译值20207.userCreateFailed + ': ' + 错误.message, 'error');
  }
}
async function 切换用户状态(唯一标识, 新状态) {
  try {
    const 响应 = await fetch(window.location.pathname + '/api/users/' + 唯一标识, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    const 数据 = await 响应.json();
    if (!响应.ok || !数据.success) {
      显示提示(数据.message || 翻译值20207.userUpdateFailed, 'error');
      return;
    }
    显示提示(翻译值20207.userUpdated, 'success');
    await 加载用户列表();
  } catch (错误) {
    显示提示(翻译值20207.userUpdateFailed + ': ' + 错误.message, 'error');
  }
}
async function 删除用户(唯一标识) {
  if (!confirm(翻译值20207.confirmDeleteUser)) return;
  try {
    const 响应 = await fetch(window.location.pathname + '/api/users/' + 唯一标识, {
      method: 'DELETE'
    });
    ...
    显示提示(翻译值20207.userDeleted, 'success');
    await 加载用户列表();
  } catch (错误) {
    显示提示(翻译值20207.userUpdateFailed + ': ' + 错误.message, 'error');
  }
...
function 复制文本(文本, 类型) {
  function 复制完成() {
    显示提示(类型 === 'uuid' ? 翻译值20207.uuidCopied : 翻译值20207.subCopied, 'success');
  }
  function 降级复制() {
    ...
    try {
      ...
      显示提示(翻译值20207.copyFailed, 'error');
    }
    document.body.removeChild(临时);
  }
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(文本).then(复制完成).catch(降级复制);
  } else {
    降级复制();
  }
}
...
async function 解析木马头部(缓冲234, 本地值233) {
  ...
  const 是否有效用户密码 = 有效用户SHA224集合 && 有效用户SHA224集合.has(密码232);
  if (密码232 !== 值224密码 && !是否有效用户密码) {
    return {
      hasError: true,
...
async function 读取扩展超文本头部(本地值180, 唯一标识字符串) {
  ...
  try {
    ...
    let 标识有效 = 验证唯一标识扩展超文本(标识175, 唯一标识174);
    if (!标识有效 && 有效用户UUID集合 && 有效用户UUID集合.size > 0) {
      try {
        标识有效 = 有效用户UUID集合.has(处理格式值(标识175));
      } catch (忽略) {}
    }
    if (!标识有效) {
      return `invalid UUID`;
...
async function 处理用户接口(请求) {
  if (!键值存储) {
    return new Response(JSON.stringify({
      success: false,
      error: 'KV存储未配置',
      message: '需要配置KV存储才能使用用户管理功能'
    }), {
      status: 503,
      headers: {
        'Content-Type': 'application/json'
  ...
  try {
    const 请求网址 = new URL(请求.url);
    const 路径部分列表 = 请求网址.pathname.split('/').filter(参数值 => 参数值);
    const 接口索引 = 路径部分列表.indexOf('api');
    const 路径段 = 接口索引 >= 0 ? 路径部分列表.slice(接口索引 + 2) : [];
    const 目标唯一标识 = 路径段.length > 0 ? 路径段[路径段.length - 1].toLowerCase() : '';
    if (请求.method === 'GET') {
      const 列表 = await 读取用户列表(true);
      const 基础地址 = 请求网址.origin;
      const 数据 = 列表.map(用户 => ({
        ...用户,
        subUrl: 基础地址 + '/' + 用户.uuid + '/sub',
        pageUrl: 基础地址 + '/' + 用户.uuid
      }));
      return new Response(JSON.stringify({
        success: true,
        count: 数据.length,
    ...
    if (请求.method === 'POST') {
      const 主体 = await 请求.json().catch(() => ({}));
      const 名称 = String(主体.name || '').trim().slice(0, 50);
      let 唯一标识 = String(主体.uuid || '').trim().toLowerCase();
      if (唯一标识 && !是否有效格式(唯一标识)) {
        return new Response(JSON.stringify({
          success: false,
          message: 'UUID格式无效'
      ...
      if (!唯一标识) 唯一标识 = 生成新唯一标识();
      if (唯一标识 === 认证令牌) {
        return new Response(JSON.stringify({
          success: false,
          message: '不能使用管理员UUID创建用户'
        }), {
          status: 400,
          headers: {
            'Content-Type': 'application/json'
      ...
      const 用户 = {
        ...
        name: 名称,
        uuid: 唯一标识,
        enabled: 主体.enabled === false ? false : true,
    ...
    if (目标唯一标识 && 是否有效格式(目标唯一标识)) {
      const 列表 = await 读取用户列表(true);
      const 索引 = 列表.findIndex(用户 => 用户.uuid === 目标唯一标识);
      if (索引 < 0) {
        return new Response(JSON.stringify({
          success: false,
          message: '用户不存在'
      ...
      if (请求.method === 'PATCH') {
        const 主体 = await 请求.json().catch(() => ({}));
        if (主体.name !== undefined) 列表[索引].name = String(主体.name).trim().slice(0, 50);
        if (主体.enabled !== undefined) 列表[索引].enabled = 主体.enabled === true || 主体.enabled === 'yes' || 主体.enabled === 'on';
        await 保存用户列表(列表);
        await 刷新有效用户集合();
        return new Response(JSON.stringify({
          success: true,
